...
loadinfo load_pe(int fd)
{
	...
	if (te->Signature!=EFI_TE_IMAGE_HEADER_SIGNATURE) ERROR("Invalid VZ signature");
	if (te->Machine!=EFI_IMAGE_FILE_MACHINE_I386 && te->Machine!=EFI_IMAGE_MACHINE_X64) ERROR("TE not i386/x86_64");
